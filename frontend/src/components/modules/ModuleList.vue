<template>
    ...
    <!-- Clickable header — same pattern as classroom groups -->
    <button type="button" @click="toggleGroup('__public__')"
        ...
        <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl
            ...
            <span class="font-dosis text-xs font-bold uppercase tracking-widest
                         text-safety-teal-700 dark:text-safety-teal-400">Public Access</span>
        </div>
    ...
    <!-- Animated collapse — reuses existing group-collapse transition -->
    <Transition name="group-collapse" @enter="onGroupEnter" @after-enter="onGroupAfterEnter" @leave="onGroupLeave">
        <div v-show="!isCollapsed('__public__')" class="overflow-hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 pb-2">
                <ModuleCard v-for="module in visibleModules(publicModules, 'public')"
                    :key="module.id" :module="module" @view="viewModule" />
            </div>
            <div class="flex justify-end">
                <button v-if="publicModules.length > SECTION_LIMIT"
                    @click.stop="toggleExpand('public')" class="show-more-btn">
