    ...
    <transition name="modal-fade">
      ...
      >
        <div class="bg-white dark:bg-[#0d0d12] border border-slate-200 dark:border-white/10 p-10 rounded-[2.5rem] max-w-sm w-full shadow-2xl animate-vessel relative overflow-hidden text-center">
          ...
          <h3 class="text-xl font-[900] text-black dark:text-white mb-2 uppercase italic tracking-tighter">Terminate?</h3>
          <p class="text-slate-500 dark:text-slate-400 mb-10 text-[10px] font-bold uppercase tracking-widest italic leading-relaxed">Disconnecting from active system link...</p>
          
          <div class="flex gap-3">
            <button @click="showLogoutModal = false"
              class="flex-1 py-4 bg-slate-100 dark:bg-white/5 text-slate-400 rounded-2xl font-black text-[10px] tracking-widest transition-all italic hover:text-black dark:hover:text-white"
            >DISMISS</button>
            <button @click="handleLogout"
              class="btn-3d-red flex-1 py-4 text-white rounded-2xl font-black text-[10px] tracking-widest transition-all uppercase"
            >Confirm</button>
          </div>
        </div>
      </div>
