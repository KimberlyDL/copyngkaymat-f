        ...
        <div v-else-if="classroomStore.classrooms.length > 0" class="space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                <div v-for="classroom in paginatedClassrooms" :key="classroom.id"
                    ...
                    class="card card-hover cursor-pointer group flex flex-col">
                    ...
                    <div class="mt-4 pt-3 border-t border-slate-100 dark:border-abyss-600 flex items-center justify-between">
                        <span v-if="isFacilitator"
                            class="inline-flex items-center gap-1.5 font-mono text-xs font-semibold px-2.5 py-1 rounded-lg bg-abyss-700/50 dark:bg-abyss-800 text-calm-lavender-500 dark:text-calm-lavender-400 border border-abyss-500 dark:border-abyss-600 tracking-widest">
                            {{ classroom.join_code }}
                        </span>
            ...
            <AppPagination v-model="currentPage" :total="classroomStore.classrooms.length" :page-size="PAGE_SIZE"
