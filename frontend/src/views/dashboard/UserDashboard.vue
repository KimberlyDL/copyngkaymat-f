<template>
    <div
        ...
        <div class="fixed top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
            <div
                class="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-600/20 rounded-full blur-[120px] mix-blend-screen dark:mix-blend-lighten opacity-50 animate-pulse">
            </div>
            <div
                class="absolute bottom-0 left-0 w-[500px] h-[500px] bg-fuchsia-600/20 rounded-full blur-[120px] mix-blend-screen dark:mix-blend-lighten opacity-50 animate-pulse delay-1000">
            </div>
        </div>
        ...
        <div class="relative z-10 space-y-10 p-6 md:p-10 animate-in fade-in duration-700">
            ...
            <header
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2.5">
                        ...
                        <span
                            class="text-[10px] font-extrabold uppercase tracking-[0.3em] text-purple-600 dark:text-purple-400">Student
                            Dashboard</span>
                    </div>
                    <h1
                        class="text-3xl md:text-4xl font-[900] text-black dark:text-white uppercase tracking-tighter leading-tight">
                        Hello, <span
                            class="bg-gradient-to-r from-purple-600 via-fuchsia-500 to-purple-500 bg-clip-text text-transparent">{{
                            userFirstName }}</span>
                    </h1>
                    <p class="text-xs font-medium text-black/50 dark:text-slate-400 tracking-tight">{{ currentDate }} •
                        {{ motivationalMessage }}</p>
                </div>

                <div class="flex gap-3">
                    <div
                        class="flex items-center gap-3 px-5 py-2.5 bg-white/60 dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm hover:shadow-purple-500/10 transition-all duration-300 group">
                        <div
                            class="p-1.5 bg-amber-500/10 rounded-full group-hover:scale-110 transition-transform duration-300">
                            <ZapIcon class="h-4 w-4 text-amber-500" />
                        </div>
                        <div class="flex flex-col">
                            <span
                                class="text-[8px] font-black uppercase text-slate-400 tracking-[0.2em] leading-none mb-0.5">Lv.</span>
                            <span class="text-lg font-[900] leading-none text-slate-800 dark:text-white">{{
                                auth.user?.gamification?.level || 1 }}</span>
                        </div>
                    </div>
                    <div
                        class="flex items-center gap-3 px-5 py-2.5 bg-white/60 dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-sm hover:shadow-orange-500/10 transition-all duration-300 group">
                        <div
                            class="p-1.5 bg-orange-500/10 rounded-full group-hover:scale-110 transition-transform duration-300">
                            <FlameIcon class="h-4 w-4 text-orange-500" />
                        </div>
                        <div class="flex flex-col">
                            <span
                                class="text-[8px] font-black uppercase text-slate-400 tracking-[0.2em] leading-none mb-0.5">Streak</span>
                            <span class="text-lg font-[900] leading-none text-slate-800 dark:text-white">{{
                                auth.user?.gamification?.streak || 0 }}</span>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div
                class="relative overflow-hidden bg-amber-50/50 dark:bg-amber-500/[0.02] backdrop-blur-xl border border-amber-200/50 dark:border-amber-500/10 rounded-[2rem] p-6 shadow-sm mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
                <div class="flex items-center justify-between mb-6">
                    <div class="flex items-center gap-4">
                        ...
                        <h3 class="text-xl font-[900] text-black dark:text-white uppercase tracking-tighter italic">
                            Latest <span class="text-amber-500">Updates</span></h3>
                    </div>
                </div>
            ...
            <div
                class="relative overflow-hidden bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[2rem] p-8 text-white shadow-xl mb-8 group">
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:scale-125 transition-transform duration-1000">
                </div>
                <div
                    class="absolute bottom-0 left-0 w-40 h-40 bg-fuchsia-500/20 rounded-full blur-2xl translate-y-1/3 -translate-x-1/3">
                </div>

                <div class="relative z-10 flex flex-col md:flex-row items-center gap-8">
                    <div
                        class="p-4 bg-white/20 backdrop-blur-md rounded-2xl border border-white/20 shadow-inner group-hover:rotate-6 transition-transform duration-500">
                        <GraduationCapIcon class="h-10 w-10 text-white" />
                    </div>
                    <div class="text-center md:text-left flex-1">
                        <div class="flex items-center justify-center md:justify-start gap-2 mb-3">
                            <span
                                class="px-2 py-0.5 rounded-md bg-white/20 text-[10px] font-black uppercase tracking-widest border border-white/10">Daily
                                Fact</span>
                        </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <router-link v-for="action in userQuickActions" :key="action.label" :to="{ name: action.name }"
                    ...
                    <div
                        :class="`p-2.5 rounded-xl ${action.bg} text-white shadow-lg ${action.shadow} group-hover:scale-110 transition-transform duration-500`">
                        <component :is="action.icon" class="h-4 w-4" />
                    </div>

                    <div>
                        <span
                            class="text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-purple-600 dark:group-hover:text-purple-400 uppercase tracking-wider transition-colors">
                            {{ action.label }}
                        </span>
                    </div>
                </router-link>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <!-- XP Card -->
                    <div
                        class="relative overflow-hidden bg-white/90 dark:bg-[#0d0d15]/60 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem] p-8 shadow-lg">

                        <div class="relative z-10">
                            <div class="flex items-center gap-3 mb-6">
                                <div class="h-6 w-1 bg-purple-600 rounded-full"></div>
                                <h3 class="text-lg font-[900] text-black dark:text-white uppercase tracking-tighter">
                                    Your Progress</h3>
                            </div>
                                ...
                                <div class="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                                            Total Experience</p>
                                        <p
                                            class="text-4xl md:text-5xl font-[900] text-black dark:text-white tracking-tighter">
                                            {{ auth.user?.gamification?.experience_points?.toLocaleString() || 0 }}
                                            <span
                                                class="text-xl text-purple-600 dark:text-purple-400 font-[800]">XP</span>
                                        </p>
                                    </div>
                                    <div class="text-left md:text-right">
                                        <p
                                            class="text-[10px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-1">
                                            Level {{ auth.user?.gamification?.level || 1 }}</p>
                                        <p class="text-2xl font-[900] text-slate-300 dark:text-slate-600">{{
                                            xpProgressPercent }}%</p>
                                    </div>
                                </div>

                                <div
                                    class="relative h-4 w-full bg-slate-100/50 dark:bg-black/40 rounded-full p-1 shadow-inner border border-slate-200/50 dark:border-white/5 overflow-hidden">
                                    <div class="h-full bg-gradient-to-r from-purple-600 via-fuchsia-500 to-indigo-500 rounded-full relative transition-all duration-1000 shadow-sm"
                                        :style="{ width: xpProgressPercent + '%' }">
                                        <div
                                            class="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-full bg-white/60 blur-[1px]">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="lg:col-span-4 space-y-8 lg:sticky lg:top-8">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between mb-8 px-2">
                            <h3
                                class="text-lg font-[900] text-black dark:text-white uppercase tracking-tighter flex items-center gap-2">
                                <TrophyIcon class="h-5 w-5 text-amber-500 fill-amber-500/20" />
                                Top <span
                                    class="bg-clip-text text-transparent bg-gradient-to-r from-amber-500 to-orange-500">Learners</span>
                            </h3>
                        </div>
                        <div class="space-y-3">
                            <div v-for="(player, index) in leaderboard.slice(0, 5)" :key="index"
                                ...
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-6 h-6 flex items-center justify-center rounded-full bg-slate-100 dark:bg-white/10 text-[10px] font-black italic text-slate-400 group-hover:bg-purple-500 group-hover:text-white transition-colors">
                                        {{ index + 1 }}
                                    </div>
                                    <span
                                        class="text-xs font-bold truncate max-w-[100px] text-slate-700 dark:text-slate-200 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{{
                                        player.name }}</span>
                                </div>
                                <span
                                    class="text-[10px] font-black text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-500/10 px-2 py-1 rounded-md">{{
                                    formatXP(player.points) }} XP</span>
                            </div>
                        </div>
                        <button @click="isLeaderboardOpen = true"
                            class="w-full mt-6 py-3.5 bg-slate-100 dark:bg-white/5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-white hover:bg-purple-600 transition-all duration-300 shadow-inner hover:shadow-lg hover:shadow-purple-500/30">View
                    ...
                    <section class="space-y-6">
                        <div class="flex items-center justify-between px-2">
                            <div class="flex items-center gap-3">
                                ...
                                <h3 class="text-lg font-[900] text-black dark:text-white uppercase tracking-tighter">
                                    Featured Learning</h3>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <a v-for="video in educationalVideos" :key="video.id" :href="video.link" target="_blank"
                                class="group relative bg-white dark:bg-[#0d0d15] border border-slate-200 dark:border-white/10 rounded-[1.5rem] overflow-hidden hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 block cursor-pointer">
                                <div class="relative h-48 w-full bg-slate-200 dark:bg-black overflow-hidden">
                                    <img :src="video.thumbnail" alt="Thumbnail"
                                        class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 bg-slate-300" />
                                    <div
                                        class="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                                        <div
                                            class="w-12 h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/40 group-hover:scale-110 transition-transform">
                                            <PlayIcon class="w-5 h-5 text-white fill-current ml-1" />
                                        </div>
                                    </div>
                                    <span
                                        class="absolute top-3 right-3 px-2 py-1 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold rounded-lg uppercase tracking-wider">{{
                                        video.duration }}</span>
                                    <span
                                        class="absolute bottom-3 left-3 px-2 py-1 bg-purple-600/90 text-white text-[9px] font-black rounded-lg uppercase tracking-wider shadow-lg">{{
                                        video.category }}</span>
                                </div>
                                <div class="p-5">
                                    <h4
                                        class="font-bold text-lg text-black dark:text-white leading-tight mb-2 line-clamp-2 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                                        {{ video.title }}</h4>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mb-4 line-clamp-2">{{
                                        video.description }}</p>
                                    <span
                                        class="text-[10px] font-black uppercase tracking-widest text-purple-600 dark:text-purple-400 group-hover:underline">Watch
                                        Now</span>
                                </div>
                            </a>
                        </div>
...
const userQuickActions = [
    { name: 'user.modules', icon: BookOpenIcon, label: 'Modules', bg: 'bg-purple-600', shadow: 'shadow-purple-500/20' },
    { name: 'user.rewards-shop', icon: LibraryIcon, label: 'Rewards', bg: 'bg-fuchsia-600', shadow: 'shadow-fuchsia-500/20' },
    { name: 'classrooms.index', icon: GraduationCapIcon, label: 'Classes', bg: 'bg-violet-600', shadow: 'shadow-violet-500/20' },
    { name: 'profile', icon: UserIcon, label: 'Profile', bg: 'bg-slate-600', shadow: 'shadow-slate-500/20' }
]
