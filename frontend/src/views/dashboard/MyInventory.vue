      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
        <div v-for="item in paginatedInventory" :key="item.id" class="item-card group">
          ...
          <div class="p-3.5 flex flex-col flex-1 gap-2">
            ...
            <div class="mt-auto pt-2.5 border-t-2 border-platinum-200 dark:border-abyss-500 flex items-center justify-between gap-1">
              <div>
                <p class="font-dosis text-[10px] font-semibold uppercase tracking-wide text-platinum-500">Unlocked</p>
                <p class="font-mplusrounded text-xs font-medium text-abyss-700 dark:text-platinum-300">
                  {{ formatDate(item.claimed_at) }}
                </p>
              </div>
              <div class="text-right">
                <p class="font-dosis text-[10px] font-semibold uppercase tracking-wide text-platinum-500">XP Value</p>
                <p class="font-dosis text-xs font-bold text-calm-lavender-600 dark:text-calm-lavender-400">
...
import { Gift, CheckCircle, PackageOpen, ArrowRight } from 'lucide-vue-next';
