  ...
  <div class="min-h-screen w-full flex items-center justify-center bg-[#fdfcff] dark:bg-[#04020a] font-['Poppins'] relative overflow-hidden p-4 transition-colors duration-700">
    ...
    <div class="w-full max-w-[500px] relative z-10 animate-in">
      <div class="bg-white/40 dark:bg-white/[0.03] backdrop-blur-3xl border border-white dark:border-white/10 p-8 sm:p-12 rounded-[3.5rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.2),inset_0_1px_2px_rgba(255,255,255,1)] dark:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)]">
        ...
        <div class="text-center mb-10">
          <h1 class="text-4xl font-[900] text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none mb-3">
            Join <span class="text-transparent bg-clip-text bg-gradient-to-br from-purple-600 via-fuchsia-500 to-fuchsia-600">Us</span>
          </h1>
          <p class="text-slate-500 dark:text-slate-400 text-sm font-medium italic">Create an account to start your journey.</p>
          <div class="h-1.5 w-20 liquid-3d-underline mt-4 rounded-full shadow-[0_10px_20px_rgba(168,85,247,0.3)] mx-auto"></div>
        </div>

        <form @submit.prevent="handleSignUp" class="space-y-6">
          <div class="space-y-2">
            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-2">Email Address</label>
            <input v-model.trim="form.email" type="email" required placeholder="name@email.com"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div class="space-y-2">
              <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-2">Password</label>
              <input v-model="form.password" type="password" required placeholder="8+ characters"
                class="w-full px-6 py-4 rounded-[1.5rem] bg-white/50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 shadow-[inset_0_1px_2px_rgba(255,255,255,0.5)] dark:shadow-none"
                :class="{ 'border-red-500/50 focus:ring-red-500/30': errors.password }" />
            </div>
            <div class="space-y-2">
              <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-2">Confirm</label>
          ...
          <div class="flex items-center gap-3 px-2 py-1">
            ...
            <label for="terms" class="text-[11px] text-slate-500 font-bold italic">
              I agree to the <span class="text-purple-600 hover:text-fuchsia-500 cursor-pointer">Terms</span> and 
              <span class="text-fuchsia-500 hover:text-purple-600 cursor-pointer">Privacy Policy</span>
            </label>
          </div>
          ...
          <button type="submit" :disabled="isLoading" 
            ...
            <span v-if="!isLoading" class="relative z-10 drop-shadow-lg">Register Now</span>
        ...
        <div class="flex items-center my-9 gap-4">
          <div class="h-[1px] flex-1 bg-slate-200 dark:bg-white/10"></div>
          <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em] italic leading-none">Or use social</span>
          <div class="h-[1px] flex-1 bg-slate-200 dark:bg-white/10"></div>
        </div>
      ...
      <div class="mt-8 text-center">
        ...
        <router-link :to="{ name: 'login' }" class="text-purple-600 dark:text-fuchsia-400 font-black uppercase tracking-[0.2em] text-[10px] hover:text-purple-500 transition-colors ml-2 underline underline-offset-4 decoration-fuchsia-500">Sign In</router-link>
      </div>
    </div>
  </div>
</template>
...
const signUpWithGoogle = async () => {
  ...
  try {
    ...
    toast.error("Google failed. Try again.");
  }
}
