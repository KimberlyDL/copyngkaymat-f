  ...
  <div class="page-wrapper animate-in">
      ...
      <div class="space-y-1.5 md:col-span-2">
        ...
        <div class="relative">
          <input
            ...
            class="input-field !pl-10 placeholder:text-platinum-700 dark:placeholder:text-platinum-400"
          />
          <SearchIcon class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-platinum-500 dark:text-platinum-400 pointer-events-none" />
        </div>
      </div>
    </div>
    ...
    <div class="table-panel">
      ...
      <div v-else-if="facilitators.length === 0" class="empty-state">
        ...
        <p class="empty-state-desc">No results match your current filters, or none have been added yet.</p>
            ...
            <tr v-for="facilitator in facilitators" :key="facilitator.id"
              ...
              <td class="td-cell pl-6">
                <div class="flex items-center gap-3">
                  <img
                    class="h-10 w-10 rounded-xl object-cover shrink-0 border-2 border-platinum-300 dark:border-abyss-500 group-hover:border-calm-lavender-200 dark:group-hover:border-calm-lavender-800/50 transition-colors"
                  ...
                  />
                  <div class="min-w-0">
                    <p class="font-dosis text-sm font-semibold text-abyss-800 dark:text-platinum-100 truncate group-hover:text-calm-lavender-600 dark:group-hover:text-calm-lavender-400 transition-colors">{{ facilitator.name }}</p>
                    <p class="font-mplusrounded text-xs text-platinum-600 dark:text-platinum-500 truncate">{{ facilitator.email }}</p>
                  </div>
                </div>
              </td>
              <td class="td-cell">
                <span :class="['badge capitalize', getRoleBadgeClass(facilitator.role)]">{{ facilitator.role }}</span>
              </td>
              <td class="td-cell">
                <div class="flex flex-col gap-1">
                  <span :class="['badge capitalize', getStatusBadgeClass(facilitator.account_status)]">{{ facilitator.account_status }}</span>
                  <span v-if="facilitator.requires_password_change" class="font-mplusrounded text-xs text-vawc-orange-600 dark:text-vawc-orange-400">Pending setup</span>
                </div>
              </td>
              <td class="td-cell">
              ...
              <td class="td-cell text-right pr-6">
                <div class="flex items-center justify-end gap-1.5">
                  <button @click="resendWelcome(facilitator)" class="icon-btn icon-btn--lavender" title="Resend welcome email"><MailIcon class="w-4 h-4" /></button>
                  <button @click="editFacilitator(facilitator)" class="icon-btn icon-btn--lavender" title="Edit facilitator"><Edit2Icon class="w-4 h-4" /></button>
                  <button @click="promptDelete(facilitator)" class="icon-btn icon-btn--red" title="Delete facilitator"><Trash2Icon class="w-4 h-4" /></button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <!-- Pagination -->
    <AppPagination v-model="currentPage" :total="pagination.total" :page-size="PAGE_SIZE" item-label="facilitators" />
...
import { UsersIcon, PlusIcon, SearchIcon, MailIcon, Edit2Icon, Trash2Icon, ChevronDown as ChevronDownIcon } from 'lucide-vue-next';
...
const resendWelcome = async (f) => {
  try { await api.post(`/api/v1/facilitators/${f.id}/resend-welcome`); toast.success(`Welcome email sent to ${f.email}`); }
  catch (error) { toast.error('Failed to send welcome email'); }
};

const editFacilitator = (f) => toast.info('Edit coming soon');

const promptDelete = (f) => { facilitatorToDelete.value = f; showDeleteModal.value = true; };
...
let searchTimeout;
const debouncedSearch = () => { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => resetAndFetch(), 500); };
const handleFacilitatorCreated = () => { showCreateModal.value = false; fetchFacilitators(); };

const getRoleBadgeClass = (role) => role === 'educator' ? 'badge-lavender' : 'badge-pink';
const getStatusBadgeClass = (status) => status === 'active' ? 'badge-teal' : 'badge-muted';
...
.th-cell { @apply px-5 py-4 font-dosis text-xs font-semibold uppercase tracking-wide text-platinum-600 dark:text-platinum-500; }
.td-cell { @apply px-5 py-4 whitespace-nowrap; }

.icon-btn { @apply p-2 rounded-lg transition-all duration-150 border-2; }
.icon-btn--lavender {
  ...
  @apply hover:bg-calm-lavender-50 dark:hover:bg-calm-lavender-900/20 hover:border-calm-lavender-200 dark:hover:border-calm-lavender-800/40 hover:text-calm-lavender-600 dark:hover:text-calm-lavender-400;
}
.icon-btn--red {
  @apply bg-platinum-200 dark:bg-abyss-600 border-platinum-300 dark:border-abyss-500 text-platinum-600 dark:text-platinum-400;
